...
class PGRService {
  ...
  async getCityExternalWebpageLink(tenantId, whatsAppBusinessNumber) {
    let url = config.egovServices.externalHost + config.egovServices.cityExternalWebpagePath + '?tenantId=' + tenantId + '&phone=+253' + whatsAppBusinessNumber;
  ...
  async getLocalityExternalWebpageLink(tenantId, whatsAppBusinessNumber) {
    let url = config.egovServices.externalHost + config.egovServices.localityExternalWebpagePath + '?tenantId=' + tenantId + '&phone=+253' + whatsAppBusinessNumber;
    let shorturl = await this.getShortenedURL(url);
    return shorturl;
  }
